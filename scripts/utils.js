...
【作成ルール・トーン＆マナー（厳守）】
1. **文字数と構成の完全固定**:
   - description（紹介文）は、必ず「2文〜3文以内（合計100文字程度）」で簡潔にまとめること。
   - 段落分け（改行）をせず、スッキリとした1つのパラグラフとして出力すること。
2. **出力禁止事項（ポエム・営業トークの禁止）**:
   - 「春夏秋冬」「秋の連休」「GW」「年末年始」などの季節や時期を限定するワードは絶対に使用しないこと（通年使える文章にすること）。
   - 「早めのご予約をおすすめします」「いかがでしょうか？」などのユーザーへの語りかけや営業トークを含めないこと。
   - エッセイ調、ポエム調の過剰な形容詞を避けること。
3. **記載すべき内容（客観的事実のみ）**:
   - アクセス（駅チカなど）、清潔感、設備（大浴場など）、コスパの良さといった「客観的な強み」のみを抽出し、端的に説明すること。
4. 「バグ級」「コスパ崩壊」「発見！」などの品のない安易な煽りワードは一切禁止します。

【出力フォーマット（必ずこのJSONのみを出力すること）】
{
  "summary": "このホテルの最大の魅力や客観的強みを端的にまとめた30〜40文字程度の短評。",
  "description": "ルールに従って抽出した客観的事実のみのオリジナル紹介文（100文字程度、2〜3文、改行なし）。"
}
`;
